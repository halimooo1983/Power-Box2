...
interface RatingFormWorkingProps {
  data: RatingSettings;
  onChange: (data: RatingSettings) => void;
}

export function RatingFormWorking({ data, onChange }: RatingFormWorkingProps) {
  ...
  const renderStars = (style: string, rating: number = data.value) => {
    const stars = [];
      ...
      <Card className="border-green-200 bg-green-50">
        <CardHeader>
          ...
          <CardDescription className="text-green-700">
            How your rating will appear on the live site
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            ...
            {!data.showInHero && !data.showInTrust && (
              <p className="text-center text-gray-500 py-4">
                Enable display locations above to see preview
              </p>
            )}
          </div>
        </CardContent>
